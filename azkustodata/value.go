...
func decodeToStruct(cols Columns, row []types.KustoValue, p interface{}) error {
	t := reflect.TypeOf(p)
	v := reflect.ValueOf(p)
...
func (f fields) convert(col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("received a field type %q we don't recognize", col.ColumnType)
}
...
func boolConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.Bool, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
...
func dateTimeConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	if !ok {
		return fmt.Errorf("Column %s is type %s was tryihng to store a KustoValue type of %T", col.ColumnName, col.ColumnType, k)
	}
	sf, _ := t.Elem().FieldByName(fieldName)
	switch {
...
func dynamicConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	case sf.Type.Kind() == reflect.Ptr && sf.Type.Elem().Kind() == reflect.Struct:
		...
		if err := json.Unmarshal([]byte(val.Value), store.Interface()); err != nil {
			return fmt.Errorf("Column %s of type dynamic could not unmarshal into the passed *struct: %s", col.ColumnName, err)
		}
	...
	case sf.Type.Kind() == reflect.Ptr && sf.Type.Elem().Kind() == reflect.Map:
		...
		if sf.Type.Elem().Key().Kind() != reflect.String {
			return fmt.Errorf("Column %s is type dymanic and can only be stored in a *map[string]interface{} or *struct", col.ColumnName)
		}
		...
		if err := json.Unmarshal([]byte(val.Value), &m); err != nil {
			return fmt.Errorf("Column %s of type dynamic could not unmarshal into the passed *map: %s", col.ColumnName, err)
		}
	...
	case sf.Type.Kind() == reflect.Map:
		...
		m := map[string]interface{}{}
		if err := json.Unmarshal([]byte(val.Value), &m); err != nil {
			return fmt.Errorf("Column %s is type dynamic and the value could not be stored in a *map[string]interface{}: %s", col.ColumnName, err)
		}
		v.Elem().FieldByName(fieldName).Set(reflect.ValueOf(m))
		return nil
	case sf.Type.Kind() == reflect.Struct:
		...
		if err := json.Unmarshal([]byte(val.Value), structPtr.Interface()); err != nil {
			return fmt.Errorf("Column %s is type dynamic and the value could not be stored in passed struct: %s", col.ColumnName, err)
		}
		v.Elem().FieldByName(fieldName).Set(structPtr.Elem())
		return nil
	}
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.Dynamic, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
...
func guidConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.GUID, struct had type %s ", col.ColumnName, fieldName, sf.Type.Name())
}
...
func intConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.Int, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
...
func longConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.Long, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
...
func realConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.Real, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
...
func stringConvert(fieldName string, col Column, k types.KustoValue, t reflect.Type, v reflect.Value) error {
	...
	return fmt.Errorf("column %s could not store in struct.%s: column was type Kusto.String, struct had base Kind %s ", col.ColumnName, fieldName, sf.Type.Kind())
}
