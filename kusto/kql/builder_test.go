...
func TestBuilder(t *testing.T) {
	tests := []struct {
		...
		{"Test simple literal ctor", NewBuilder("foo"), "foo"},
		{"Test add literal", NewBuilder("foo").AddLiteral("bar"), "foobar"},
		{
			"Test add int",
			NewBuilder("MyTable | where i != ").AddInt(32).AddLiteral(" ;"),
			"MyTable | where i != int(32) ;",
		},
		{
			"Test add long",
			NewBuilder("MyTable | where i != ").AddLong(32).AddLiteral(" ;"),
			"MyTable | where i != long(32) ;",
		},
		{
			"Test add real",
			NewBuilder("MyTable | where i != ").AddReal(32.5).AddLiteral(" ;"),
			"MyTable | where i != real(32.5) ;",
		},
		{
			"Test add bool",
			NewBuilder("MyTable | where i != ").AddBool(true).AddLiteral(" ;"),
