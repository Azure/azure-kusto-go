...
type querier interface {
	Query(context.Context, string, data.Statement, ...data.QueryOption) (*data.RowIterator, error)
}
...
type NodeInfo struct {
	stmt    *kql.Builder
	querier querier // This can be a fakeQuerier or *data.Client
}

// New is the constructor for NodeInfo.
func New(client *data.Client) (*NodeInfo, error) {
	return &NodeInfo{
		querier: client,
...
func (n *NodeInfo) Node(ctx context.Context, id int64) (NodeRec, error) {
	iter, err := n.querier.Query(ctx, "db", n.stmt, data.QueryParameters(kql.NewParameters().AddLong("ParamNodeId", id)))
	if err != nil {
		return NodeRec{}, err
	}
...
func (f *fakeQuerier) Query(_ context.Context, _ string, passedQuery data.Statement, _ ...data.QueryOption) (*data.RowIterator, error) {
...
func ExampleMockRows() { // nolint:govet // Example code
	...
	for _, test := range tests {
		...
		m, err := data.NewMockRows(columns)
		if err != nil {
