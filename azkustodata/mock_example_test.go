...
type querier interface {
	Query(context.Context, string, azkustodata.Statement, ...azkustodata.QueryOption) (*azkustodata.RowIterator, error)
}
...
func New(client *azkustodata.Client) (*NodeInfo, error) {
	return &NodeInfo{
		querier: client,
...
func (n *NodeInfo) Node(ctx context.Context, id int64) (NodeRec, error) {
	iter, err := n.querier.Query(ctx, "db", n.stmt, azkustodata.QueryParameters(kql.NewParameters().AddLong("ParamNodeId", id)))
	if err != nil {
		return NodeRec{}, err
	}
...
func (f *fakeQuerier) Query(_ context.Context, _ string, passedQuery azkustodata.Statement, _ ...azkustodata.QueryOption) (*azkustodata.RowIterator, error) {
	if passedQuery.String() != f.expectQuery {
		panic("we expect the query to be " + f.expectQuery)
	}

	ri := &azkustodata.RowIterator{}
	_ = ri.Mock(f.mock)
	return ri, nil
}

func ExampleMockRows() { // nolint:govet // Example code
	now := time.Now()
	...
	for _, test := range tests {
		...
		m, err := azkustodata.NewMockRows(columns)
		if err != nil {
		...
		case err == nil && test.err:
			panic(fmt.Sprintf("TestNodeInfo(%s): got err == nil, want err != nil", test.desc))
			continue
		case err != nil && !test.err:
			panic(fmt.Sprintf("TestNodeInfo(%s): got err == %s, want err == nil", test.desc, err))
			continue
		case err != nil:
			continue
		}
		...
		if diff := pretty.Compare(test.want, got); diff != "" {
			panic(fmt.Sprintf("TestNodeInfo(%s): -want/+got:\n%s", test.desc, diff))
		}
	}
}
