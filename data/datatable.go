...
func (dt *dataTable) unmarshalAttributes(m map[string]interface{}) (err error) {
	...
	if ft, ok := m[kFrameType].(string); !ok {
		...
		return errors.E(dt.op, errors.KInternal, fmt.Errorf("dataTable.Unmarshal expected FrameType == 'DataTable' but got '%s'", ft))
	} else {
...
func (dt *dataTable) unmarshalColumns(cols []interface{}) (err error) {
	for _, inter := range cols {
		...
		if !ok {
			return errors.E(dt.op, errors.KInternal, fmt.Errorf("dataTable.Columns had entry with .ColumnType set to a %T type", m[kColumnType]))
		}
		col := Column{
			ColumnName: cn,
			ColumnType: ct,
...
func (dt *dataTable) unmarshalRows(rows []interface{}) (e error) {
	for x, inter := range rows {
		...
		for i, raw := range inter.([]interface{}) {
			convertFn := conversion[dt.Columns[i].ColumnType]
			if convertFn == nil {
				return errors.E(dt.op, errors.KInternal, fmt.Errorf("in row %d, column %s: had unsupported type %s ", x, dt.Columns[i].ColumnName, dt.Columns[i].ColumnType))
			}
			cell, err := convertFn(raw)
			if err != nil {
				return errors.E(dt.op, errors.KInternal, fmt.Errorf("in row %d, column %s, conversion error: %s", x, dt.Columns[i].ColumnName, err))
			}
