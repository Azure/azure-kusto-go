...
var (
	pTableStmtOld = azkustodata.NewStmt("table(tableName)").MustDefinitions(
	...
	tests := []struct {
		...
		},
		{
			...
			stmt: pTableStmtOld.Add(" | where  vnum == num and vdec == dec and vdate == dt and vspan == span and tostring(vobj) == tostring(obj) and vb == b and vreal" +
				...
				MustParameters(azkustodata.NewParameters().Must(azkustodata.
					QueryValues{
...
func createIngestionTable(t *testing.T, client *azkustodata.Client, tableName string, isAllTypes bool) error {
	return createIngestionTableWithDB(t, client, testConfig.Database, tableName, isAllTypes)
}

func createIngestionTableWithDB(t *testing.T, client *azkustodata.Client, database string, tableName string, isAllTypes bool) error {
...
func createIngestionTableWithDBAndScheme(t *testing.T, client *azkustodata.Client, database string, tableName string, isAllTypes bool, scheme string) error {
