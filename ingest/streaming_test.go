...
import (
	...
	"github.com/Azure/azure-kusto-go/ingest/internal/gzip"
	"github.com/Azure/azure-kusto-go/ingest/internal/properties"
	"github.com/google/uuid"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

type streamIngestFunc func(ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string, clientRequestId string) error
type testStreamIngestFunc func(t *testing.T, ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string,
...
func (f fakeStreamIngestor) StreamIngest(ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string, clientRequestId string) error {
...
func TestStreaming(t *testing.T) {
	...
	compressedBuffer.Reset(io.NopCloser(bytes.NewReader(internalData)))
	...
	tests := []struct {
	...
	}{
		{
			...
			options: []FileOption{},
			onStreamIngest: func(t *testing.T, ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string,
				clientRequestId string) error {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			streamIngestor := fakeStreamIngestor{
				onStreamIngest: func(ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string, clientRequestId string) error {
