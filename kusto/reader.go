				...
				if len(rowBuffer) > 0 {
					...
					for _, s := range rowBuffer {
						if s.inRows != nil {
							...
							for k, values := range s.inRows {
								select {
								case <-r.ctx.Done():
								case r.rows <- Row{Values: values, Replace: k == 0 && s.inTableFragmentType == "DataReplace"}:
								}
							}
						...
						if s.inRowErrors != nil {
							for _, e := range s.inRowErrors {
