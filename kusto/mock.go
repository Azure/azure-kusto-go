...
func newColumnMap(cols Columns) columnMap {
...
func NewMockRows(columns Columns) (*MockRows, error) {
...
func (m *MockRows) nextRow() (*Row, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func (m *MockRows) Row(row types.KustoValues) error {
	if len(row) == 0 {
		return fmt.Errorf("cannot add an empty types.KustoValues")
	}
