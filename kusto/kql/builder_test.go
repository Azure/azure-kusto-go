...
func TestBuilder(t *testing.T) {
	tests := []struct {
		...
		{
			"Test add int",
			NewStatementBuilder("MyTable | where i != ").AddInt(32),
			"MyTable | where i != int(32)",
		},
		{
			"Test add long",
			NewStatementBuilder("MyTable | where i != ").AddLong(32),
			"MyTable | where i != long(32)",
		},
		{
			"Test add real",
			NewStatementBuilder("MyTable | where i != ").AddReal(32.5),
			"MyTable | where i != real(32.5)",
		},
		{
			"Test add bool",
			NewStatementBuilder("MyTable | where i != ").AddBool(true),
