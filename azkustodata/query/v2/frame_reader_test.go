...
func TestDecodeValidFrames(t *testing.T) {
	...
	require.Equal(t, 0, propertiesTable.Header.TableId)
	require.Equal(t, "QueryProperties", propertiesTable.Header.TableKind)
	require.Equal(t, "@ExtendedProperties", propertiesTable.Header.TableName)
	...
	require.Equal(t, 0, len(tableCompletion.OneApiErrors))
	...
	require.Equal(t, "QueryCompletionInformation", queryCompletionInformationTable.Header.TableKind)
	require.Equal(t, "QueryCompletionInformation", queryCompletionInformationTable.Header.TableName)
	require.Equal(t, 2, queryCompletionInformationTable.Header.TableId)
...
func TestReadFramesWithErrors(t *testing.T) {
	...
	reader := bytes.NewReader([]byte(partialErrors))
	f, err := newFrameReader(reader)
	require.NoError(t, err)
	...
	require.Equal(t, 1, tableHeader.TableId)
	require.Equal(t, "PrimaryResult", tableHeader.TableKind)
	require.Equal(t, "PrimaryResult", tableHeader.TableName)
	require.Equal(t, 1, len(tableHeader.Columns))
	require.Equal(t, "A", tableHeader.Columns[0].Name())
	require.Equal(t, types.Column("int"), tableHeader.Columns[0].Type())
	...
	name, err := tableFragment.Rows[0].IntByName("A")
	require.Equal(t, int32(1), *name)
	require.NoError(t, err)
	...
	require.NoError(t, f.unmarshal(&tableCompletion))
	require.Equal(t, 1, tableCompletion.TableId)
	require.Equal(t, 1, tableCompletion.RowCount)
	require.Equal(t, 1, len(tableCompletion.OneApiErrors))
	require.Equal(t, "LimitsExceeded", tableCompletion.OneApiErrors[0].ErrorMessage.Code)
}
...
func TestReadFramesWithInvalidInput(t *testing.T) {
	...
	require.ErrorContains(t, f.advance(), "EOF")
}
...
func TestReadFramesWithInvalidFrameType(t *testing.T) {
	...
	require.ErrorContains(t, f.validateDataSetHeader(), "Expected DataSetHeader, got InvalidFrameType")
}
...
func TestReadFramesWithInvalidFrame(t *testing.T) {
	...
	require.ErrorContains(t, f.validateDataSetHeader(), "Expected false, got invalid")
}
