...
// mgmter is a private interface that allows us to write hermetic tests against the data.Client.Mgmt() method.
...
func (m *Manager) AuthContext(ctx context.Context) (string, error) {
	...
	var rows *data.RowIterator
	retryCtx := backoff.WithContext(InitBackoff(), ctx)
	err := backoff.Retry(func() error {
		var err error
		rows, err = m.client.Mgmt(ctx, "NetDefaultDB", kql.New(".get kusto identity token"), data.IngestionEndpoint())
		if err == nil {
			return nil
		}
...
// fetch makes a data.Client.Mgmt() call to retrieve the resources used for Ingestion.
func (m *Manager) fetch(ctx context.Context) error {
	...
	err := backoff.Retry(func() error {
		var err error
		rows, err = m.client.Mgmt(ctx, "NetDefaultDB", kql.New(".get ingestion resources"), data.IngestionEndpoint())
		if err == nil {
			return nil
		}
		if httpErr, ok := err.(*kustoErrors.HttpError); ok {
