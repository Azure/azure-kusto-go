...
import (
	...
	"github.com/Azure/azure-kusto-go/ingest/internal/queued"
...
type streamIngestor interface {
	io.Closer
	StreamIngest(ctx context.Context, db, table string, payload io.Reader, format data.DataFormatForStreaming, mappingName string, clientRequestId string) error
}
...
func NewStreaming(client QueryClient, db, table string) (*Streaming, error) {
	streamConn, err := data.NewConn(removeIngestPrefix(client.Endpoint()), client.Auth(), client.HttpClient(), client.ClientDetails())
	if err != nil {
		return nil, err
	}
